...
import type { TranslationOutcome, TranslationProvider, TranslationRequest } from './types.ts'
...
export function canTranslateRequest(request: TranslationRequest): TranslationOutcome | null {
  ...
  if (!isSupportedLanguage(request.sourceLanguage)) return { ok: false, code: 'invalid-response' }
  if (!isSupportedLanguage(request.targetLanguage)) return { ok: false, code: 'invalid-response' }
  if (request.sourceLanguage === request.targetLanguage) return { ok: false, code: 'same-language' }
  if (text.length > MAX_TRANSLATION_CHARS) return { ok: false, code: 'too-long' }
  return null
}

/**
 * Product engine. Independent of keyboard-layout correction.
 * The provider is the only I/O; the engine never talks to Groq.
 */
export class TranslationEngine {
  ...
  async translate(request: TranslationRequest): Promise<TranslationOutcome> {
    ...
    const normalized: TranslationRequest = {
      ...request,
      text: normalizeTranslationText(request.text),
    }
    return this.provider.translate(normalized)
  }
}
